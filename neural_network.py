...
def our_function(x) -> float:
    ...
    output[x > 0.1] = 1
    return output
# np.power(x, 3) + 2 * np.power(x, 2) + 10
...
class output_layer:
    ...
    def dSSRdb(self, observed):
        return np.sum(-2 * (observed - self.inputs))

    def backward(self, dSSR, learning_rate):
        step_size_b = dSSR * learning_rate
        self.biases -= step_size_b
        step_size_w = self.dSSRdw(dSSR) * learning_rate
        self.weights -= step_size_w
        return self.weights
...
class NeuralNetwork:
    ...
    def dSSR(self, expected, predicted):
        return np.sum(-2 * (expected - predicted))
...
if __name__ == "__main__":
    ...
    nn.train(inputs.reshape([input_size, num_of_data]), y, epochs=200)
