...
if __name__ == "__main__":
    ...
    X_file = sys.argv[1]
    y_file = sys.argv[2]
    X_rot_file = sys.argv[3]
    y_rot_file = sys.argv[4]
    X = np.load(X_file)
    y = np.load(y_file)
    X_rot = np.load(X_rot_file)
    y_rot = np.load(y_rot_file)

    training_N = int(X.shape[0])  # int(33000)
    X_data = tf.convert_to_tensor(X[:training_N, :])
    y_data = tf.convert_to_tensor(y[:training_N, :])
    X_validate = tf.convert_to_tensor(X_rot)
    y_validate = tf.convert_to_tensor(y_rot)
